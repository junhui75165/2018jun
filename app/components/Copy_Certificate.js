...
(function () {
    ...
    let { Card, Col, Row ,Button,Table,Input ,DatePicker,
        Popconfirm,Pagination,message,Modal,Icon,Select   } = require('antd');
    const {Option} = Select;
    const RangePicker = DatePicker.RangePicker;
    let {toThousands,getDateVnum,printVoucher,GetLocalStorage} = require('../tool');
    class Copy_Certificate extends React.Component{
        constructor(props){
            super(props);
            this.state = {
                editModal:false,
                ...
                data : [],
                searchDate: [moment().startOf('year'), moment().endOf('year')],
                searchNum:'',
                summary:'',
                checkInfo:{},
                editInfo:{},
        ...
        componentWillMount() {
            ...
            let cb = (data)=>{
                this.setState({
                    searchDate:[moment(data.date_).startOf('year'), moment(data.date_).endOf('year')],
                    dateVnum:data
                })
            };
            getDateVnum(cb);
        ...
        setTable(props){
            ...
            list.map((con,i)=>{
                ...
                data[i].key = i;
                data[i].vnum = con.vnum;
                data[i].account = con.gl_amount;
                data[i].summary = con.memo_;
        ...
        render() {
            ...
            const columns = [
                {
                    ...
                    dataIndex: 'watch',
                    key: 'watch',
                    render:(text, record, index)=>(<Button onClick={this.showModal.bind(this,'check',record,index)} shape="circle" icon="search"/>)
                },{
                    title: '复制',
                    dataIndex: 'edit',
                    key: 'edit',
                    render:(text, record, index)=>(<Button onClick={this.showModal.bind(this,'edit',record)} shape="circle" icon="copy"/>)
                },{
                    title: '作废',
                    dataIndex: 'delete',
                    key: 'delete',
                    render:(text, record, index)=>(<Popconfirm placement="leftTop" title="是否删除凭证" onConfirm={this.deleteItem.bind(this,record)} okText="确定" cancelText="取消">
                        <Button shape="circle" icon="delete"/></Popconfirm>)
                }
            ];
            const ranges = [
                {
            ...
            return (
                <div className="gen-certificate">
                    <Card title={<h1 className="certificate-title">复制凭证</h1>}
                          bordered={true}>
                        ...
                        <Modal visible={this.state.checkModal}
                            ...
                            <div id="section-to-print" className="certificate-table">
                                <Table bordered pagination={false} size="small"
                                       dataSource={this.state.checkInfo.topTable} columns={this.state.checkInfo.topLabel} />
                                <Table style={{marginTop:'1rem'}} bordered pagination={false} size="small"
                                       dataSource={this.state.checkInfo.contentTable} columns={this.state.checkInfo.contentLabel} />
    ...
    module.exports = Copy_Certificate;
})();